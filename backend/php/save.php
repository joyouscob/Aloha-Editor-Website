...
try {
  ...
  if (!empty($_POST['changes'])) {
    foreach ($_POST['changes'] as $change) {
      ...
      $select = $db->prepare('SELECT id FROM team WHERE id=? LIMIT 1');
      $select->execute(array(
        $rowId
      ));
      
      if ($row = $select->fetch()) {
        $query = $db->prepare('UPDATE team SET `' . $colMap[$colId] . '` = :newVal WHERE id = :id');
      } else {
        $query = $db->prepare('INSERT INTO team (id, `' . $colMap[$colId] . '`) VALUES(:id, :newVal)');
      }
    ...
    $select = $db->prepare('DELETE FROM team');
    $select->execute();
    ...
    for ($r = 0, $rlen = count($_POST['data']); $r < $rlen; $r++) {
      ...
      for ($c = 0, $clen = count($_POST['data'][$r]); $c < $clen; $c++) {
        ...
        if ($row = $select->fetch()) {
          $query = $db->prepare('UPDATE team SET `' . $colMap[$c] . '` = :newVal WHERE id = :id');
        } else {
          $query = $db->prepare('INSERT INTO team (id, `' . $colMap[$c] . '`) VALUES(:id, :newVal)');
        }
        $query->bindValue(':id', $rowId, PDO::PARAM_INT);
        $query->bindValue(':newVal', $newVal, PDO::PARAM_STR);
        $query->execute();
